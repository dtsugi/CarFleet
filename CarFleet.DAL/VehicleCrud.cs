...
namespace CarFleet.DAL
{
    public class VehicleCrud : CrudEntityBase<VehicleEntity>
    {
        public override void Map(IDataRecord record, VehicleEntity entity)
        {
            ...
            //if (base.HasValueRecord(record["odometer"])) { entity.Odometer = (int)base.GetValueRecord(record["odometer"], Utils.Constants.DATA_TYPES.FLOAT); }
            if (base.HasValueRecord(record["odometer"]))
            {
                entity.Odometer = float.Parse(base.GetValueRecord(record["odometer"], Utils.Constants.DATA_TYPES.FLOAT).ToString(), System.Globalization.CultureInfo.InvariantCulture);
            }
        ...
        public List<VehicleEntity> Select(int? id, int? idCompany, int? idFleet, int? idVehicleType, int? idDriver)
        {
            using (var command = base.GetContextConnection.CreateCommand())
            {
                ...
                command.AddParameter("ID_VEHICLETYPE_OUT", SqlDbType.Int, idVehicleType);
                command.AddParameter("ID_DRIVER_OUT", SqlDbType.Int, idDriver);
                command.CommandText = "stp_VehicleSelect";
